...
// Rank thresholds align with the capsule reward tiers so the rank a
// player earns is a direct read of how many capsules they got:
//   SILVER (15K)  = 1 capsule
//   GOLD   (30K)  = 2 capsules
//   COSMIC (50K)  = 3 capsules
// BRONZE sits below the first capsule threshold, WOOD is the floor.
const RANK_CONFIG = [
    { threshold: 50000, label: "COSMIC", color: "#B366FF", accent: "#1a0533", glow: "rgba(179,102,255,0.6)", stars: 5, icon: "🌌" },
    { threshold: 30000, label: "GOLD", color: "#FFE048", accent: "#2a1a00", glow: "rgba(255,224,72,0.5)", stars: 4, icon: "🥇" },
...
export default function GameOver({ state, userProfile, onPlayAgain, onGoHome, onRequestLogin, capsuleEarned, onOpenPinBook, matchId }: GameOverProps) {
    ...
    const handleShareX = () => {
        ...
        const shareUrl = `https://pindropgame.com/?ref=${encodeURIComponent(user)}&user=${encodeURIComponent(user)}&score=${score}&capsules=${capsuleCount}`;
